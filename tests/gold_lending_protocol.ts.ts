...
describe("sol_project_v2", () => {
  ...
  let goldAssetConfig: anchor.web3.PublicKey;
  let mockPythGold: anchor.web3.Keypair;
  ...
  it("Adds Gold as a supported asset", async () => {
    ...
    const config = await program.account.assetConfig.fetch(goldAssetConfig);
    assert.ok(config.ltv.toNumber() === 8000);
  });
